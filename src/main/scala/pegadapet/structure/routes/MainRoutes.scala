...
import pegadapet.structure.directives.LoggingDirectives
import pegadapet.structure.exceptions.DefaultExceptionsHandler
import pegadapet.structure.injection.ApiModule

import scala.concurrent.ExecutionContext

class MainRoutes(implicit system: ActorSystem[_], module: ApiModule) extends DefaultExceptionsHandler with BasicDirectives with RouteDirectives with LoggingDirectives {
  ...
  def routes(): Route = logPersonalized {
    Route.seal {
      options {
